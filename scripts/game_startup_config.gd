...
const POISON_BOARD_SIZES: Array[int] = [3, 4, 5]
...
static func build_poison_config(word_set: Dictionary, board_size: int, players: Array[Dictionary], initial_health: int, poison_count: int) -> Dictionary:
	return {
		"game_type": "word_poison",
		"word_set": word_set.duplicate(true),
		"board_size": normalize_board_size(board_size, POISON_BOARD_SIZES, 4),
		"players": normalize_players(players),
...
static func validate_poison_config(config: Dictionary) -> Dictionary:
	...
	return {
		"game_type": "word_poison",
		"word_set": word_set,
		"board_size": normalize_board_size(int(config.get("board_size", 4)), POISON_BOARD_SIZES, 4),
		"players": players,
