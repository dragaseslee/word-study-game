...
func parse_word_file(file_path: String) -> Array[Dictionary]:
	if _cache.has(file_path):
		return (_cache[file_path] as Array).duplicate(true)
	var result := _parse_word_file_impl(file_path)
	_cache[file_path] = result.duplicate(true)
	return result
...
func invalidate_cache(file_path: String = "") -> void:
...
func _parse_word_file_impl(file_path: String) -> Array[Dictionary]:
